...
const CharacterSelect: React.FC<CharacterSelectProps> = ({ onSelectComplete, onBack }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 text-white overflow-hidden p-4">
      {/* Back Button */}
      <button
        onClick={onBack}
      ...
      <div className="w-full max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full mb-6">
          {characters.map(character => {
            ...
            return (
              <div 
                key={character.id}
                className={`relative border-2 rounded-lg p-4 transition-all duration-300 transform cursor-pointer h-96 ${
                  isSelected 
                  ...
                  {!isUnlocked && (
                    <div className="absolute inset-0 bg-black/70 flex items-center justify-center">
                      <div className="text-center">
                        ...
                        {isPremium ? (
                          <div className="text-yellow-400 font-bold text-xs">Premium Required</div>
                        ) : (
        ...
        <div className="relative text-center">
          <button 
            type="button"
            className={`px-16 py-4 bg-gradient-to-r from-yellow-500 via-orange-500 to-red-500 hover:from-yellow-400 hover:via-orange-400 hover:to-red-400 rounded-xl font-bold text-2xl text-black transition-all duration-300 cursor-pointer select-none transform hover:scale-110 shadow-2xl relative overflow-hidden ${
